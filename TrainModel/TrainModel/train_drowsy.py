...
Cách dùng (ví dụ):
  ...
  # Chỉ build dataset từ tất cả nguồn (Open_Eyes, Closed_Eyes, Webcam, train_vid)
  python train_drowsy.py --prepare

  # Chỉ build dataset từ NEW_REAL (Webcam) để fine-tune
  python train_drowsy.py --prepare --new-real-only

  # Train từ đầu (dùng yolov8n.pt)
  python train_drowsy.py --train --epochs 100

  # Fine-tune từ best_drowsy.pt + dataset hiện tại (ví dụ vừa build từ NEW_REAL)
  python train_drowsy.py --train --finetune --epochs 30

  # Build + train + export ONNX
...
NEW_REAL = Path("/home/gess/Documents/sub/TrainModel/Webcam/").resolve()
...
class DatasetBuilder:
    img_size: int = 640
    new_real_only: bool = False  # <<<<<< THÊM CỜ NÀY
    ...
    def _collect_images(self) -> List[Tuple[Path, int]]:
        ...
        if not self.new_real_only:
            # Dùng cả Open_Eyes + Closed_Eyes + NEW_REAL
        ...
        # Nếu new_real_only=True -> chỉ dùng NEW_REAL
        if NEW_REAL.exists():
    ...
    def _frames_from_videos(self) -> List[Tuple[Path, int]]:
        # Nếu chỉ dùng NEW_REAL thì bỏ qua VIDEO
        if self.new_real_only:
            return []
...
class Trainer:
    ...
    def __post_init__(self):
        ...
        best_path = MODEL_DIR / "best_drowsy.pt"
        if self.finetune and best_path.exists():
            print(f"🔁 Finetune từ model cũ: {best_path}")
            self.weights = str(best_path)
        else:
            w_path = Path(self.weights)
            if w_path.exists():
                print(f"🚀 Loading custom weights: {w_path}")
                self.weights = str(w_path)
            else:
                print(f"🚀 Loading base model: {self.weights}")
    ...
    def train(self, epochs: int = 100, batch: int = 8, workers: int = 4):
        ...
        if self.finetune and self.weights.endswith("best_drowsy.pt"):
            lr0 = 0.001
            mosaic = 0.2
            print("💡 Finetune mode: giảm lr0 xuống 0.001, mosaic=0.2")
        else:
            lr0 = 0.002
            mosaic = 0.3
...
def parse_args():
    ...
    ap.add_argument("--epochs", type=int, default=100, help="Số epoch train")
    ap.add_argument("--batch", type=int, default=8, help="Batch size")
    ap.add_argument("--weights", type=str, default="yolov8n.pt",
                    help="Weights khởi tạo (nếu finetune sẽ ưu tiên best_drowsy.pt)")
    ap.add_argument("--finetune", action="store_true",
                    help="Dùng best_drowsy.pt (nếu có) để train tiếp")

    # 🔥 Cờ mới: chỉ lấy data từ NEW_REAL/Webcam
    ap.add_argument("--new-real-only", action="store_true",
                    help="Chỉ dùng ảnh từ NEW_REAL/Webcam để build dataset")
...
def main():
    ...
    if args.prepare:
        builder = DatasetBuilder(img_size=args.imgsz,
                                 new_real_only=args.new_real_only)
        builder.build()
